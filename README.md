...
- **Modular React + TypeScript Dashboard:** Fast, responsive UI with bespoke Light/Dark Green structured CSS theme, real-time SRE incident streaming, 1-click approvals, and system maintenance controls.
...
│   │   ├── views/        # Tab views (ActiveIncidents, UpgradeHub, SpendTracker, etc.)
│   │   ├── hooks/        # Custom state & polling hooks
...
MonitorBot features a comprehensive test suite (129 unit and integration tests with **80% code coverage**).

For full details, see the dedicated [Testing Guide](TESTING.md).

```bash
# Run unit & integration test suite (safely isolated, no live network/agent calls):
pytest -m "not live"

# Run tests with statement coverage report:
pytest -m "not live" --cov=app --cov-report=term-missing

# Run live E2E integration tests (explicit execution only):
