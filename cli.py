...
from app.database import init_db, get_setting, set_setting, check_maintenance_status
...
def print_help():
    ...
    print("Usage:")
    print("  python3 cli.py status               - View current silent mode, autopilot, and maintenance settings")
    ...
    print("  python3 cli.py pause [duration]     - Enable maintenance mode (pause monitoring; duration: '30m', '2h', or default 'indefinite')")
    print("  python3 cli.py resume               - Disable maintenance mode (resume monitoring)")
...
def main():
    ...
    if cmd == "help":
        ...
        is_maint, reason = check_maintenance_status()
        maint_str = f"ACTIVE ({reason})" if is_maint else "INACTIVE"
        print(f"Silent Mode (Monitoring Only): {silent.upper()}")
        print(f"Autopilot (Auto-Fix):         {autopilot.upper()}")
        print(f"Maintenance Mode:              {maint_str}")
    elif cmd == "silent":
        if len(sys.argv) < 3:
        ...
        if val == "indefinite":
            ...
            print("Maintenance Mode ENABLED indefinitely (Monitoring paused)")
        else:
            import re
            from datetime import datetime, timedelta
            match = re.match(r'^(\d+)([mh])$', val)
            if not match:
                print("Error: Invalid duration format. Use '30m', '2h', or 'indefinite'.")
                sys.exit(1)
            ...
            print(f"Maintenance Mode ENABLED until {iso_str} (Monitoring paused for {val})")
            
    elif cmd == "resume":
        set_setting("maintenance_mode", "false")
        print("Maintenance Mode DISABLED (Monitoring resumed)")
