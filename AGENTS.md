...
### 5. Testing Safety & Runaway Process Prevention
- **Always Exclude Live Markers in Automated Runs:** When running `pytest`, **always** run with `-m "not live"`:
  ```bash
  pytest -m "not live"
  ```
  Tests in `test_live_executors.py` invoke live CLI binaries (`agy`, `opencode`) which can spawn interactive or long-running child processes.
- **Mock Subprocesses & Network Calls:** All standard unit/integration tests must mock `subprocess.run`, `subprocess.Popen`, and external HTTP requests.
- **Check for Orphaned Processes:** After test runs, verify no background pytest or python processes are lingering:
  ```bash
  ps aux | grep pytest
  ```
